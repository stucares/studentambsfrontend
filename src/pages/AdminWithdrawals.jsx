...
const AdminWithdrawals = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error(error.userMessage || 'Unable to load withdrawals. Please refresh the page.');
      setLoading(false);
    }
  };
  ...
  const handleUpdateStatus = async (id, status, transactionId = '') => {
    ...
    try {
      ...
      toast.error(error.userMessage || 'Unable to update withdrawal status. Please try again.');
    } finally {
  ...
  const handleUpdateSettings = async (e) => {
    ...
    try {
      ...
      toast.error(error.userMessage || 'Unable to save settings. Please try again.');
    } finally {
        ...
        <div className="flex gap-2">
          {['all', 'pending', 'processing', 'completed', 'rejected'].map((status) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg capitalize transition-all ${filter === status
                      ...
                      <div className="flex gap-2">
                        {withdrawal.status === 'pending' && (
                          <>
                            <button
                              ...
                              className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              {updatingStatus[withdrawal.id] === 'processing' ? 'Processing...' : 'Process'}
                            </button>
                            <button
                              onClick={() => handleUpdateStatus(withdrawal.id, 'rejected')}
                              disabled={!!updatingStatus[withdrawal.id]}
                              className="px-3 py-1 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              {updatingStatus[withdrawal.id] === 'rejected' ? 'Rejecting...' : 'Reject'}
                            </button>
                          </>
                        )}
                        {withdrawal.status === 'processing' && (
                          <button
                            ...
                            disabled={!!updatingStatus[withdrawal.id]}
                            className="px-3 py-1 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {updatingStatus[withdrawal.id] === 'completed' ? 'Completing...' : 'Complete'}
                          </button>
                        )}
                      </div>
                    </td>
